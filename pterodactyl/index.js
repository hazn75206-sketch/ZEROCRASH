...
const wsPort = process.env.PORT || process.env.SERVER_PORT || 4004;
server.listen(wsPort, () => {
  console.log(`🟣 Server running on http://localhost:${wsPort}`);
});

const PORT = process.env.PORT || process.env.SERVER_PORT || 4004;
...
// === ZEROCRASH MERGE: load db.json (bcrypt users) and merge into loadDatabase ===
const ZC_DB_FILE = path.join(__dirname, 'db.json');
const bcrypt = (() => { try { return require('bcryptjs'); } catch(e){ return null; } })();
function loadZC() {
  try {
    if (fs.existsSync(ZC_DB_FILE)) {
      const zc = JSON.parse(fs.readFileSync(ZC_DB_FILE,'utf8'));
      if (zc.users && zc.users.length) return zc;
    }
  } catch(e){}
  return null;
}
...
loadDatabase = function() {
  ...
  if (zc && zc.users) {
    ...
    for (const u of zc.users) {
      if (!existing.has(u.username)) {
        data.push(u);
      } else {
        // merge: keep mantax plain but also add bcrypt password if missing
        const idx = data.findIndex(x=>x.username===u.username);
        if (idx!==-1 && !data[idx].key && u.key) data[idx].key = u.key;
      }
    }
  }
  return data;
};
// Patch find to support bcrypt
const _origFind = (typeof findByKey !== 'undefined' ? findByKey : null);
// Add helper for ZC validate with bcrypt
function findByPassWithBcrypt(username, password) {
  const db = loadDatabase();
  ...
  const news = [
    {
      ...
      title: "SixEyesX",
      desc: "Buy Acces Chat @ikyhousting x @vizz_Newszera x @Netchilds"
    },
    {
      image: "https://files.catbox.moe/jpmswz.jpg",
      title: "SixEyesX",
      desc: "Join channel https://t.me/+6DCMrWanWtI5ZmQ1"
    }
  ];
...
// === ZEROCRASH DOWNLOADER (yt-dlp) - Mori universal ===
let ytdlpReady = null;
...
async function ytdlpJson(clean, format){
  const { exec } = require('child_process'); const util=require('util'); const execAsync=util.promisify(exec);
...
function pickVideoUrl(j){
  ...
  if(j.requested_formats && j.requested_formats.length){ const p=j.requested_formats.find(f=>f.url && f.acodec!=='none')||j.requested_formats[0]; return p.url; }
  return null;
}
...
app.get('/api/d/universal', async (req,res)=>{
  const url=req.query.url; if(!url) return res.json({status:false,message:'missing url'});
  const platform=detectPlatform(url);
  if(platform==='spotify'||platform==='applemusic') return res.json({status:false,platform,message:'Audio platform segera hadir'});
  try{
    const clean=String(url).replace(/"/g,'').split(' ')[0].split('\n')[0];
    let j; try{ j=await ytdlpJson(clean,'best[ext=mp4]/best'); }catch(_){ j=await ytdlpJson(clean); }
    const items=[]; const pushUrl=(u,k)=>{ if(u&&typeof u==='string'&&u.startsWith('http')) items.push({url:u,kind:k||(/(\.jpg|\.jpeg|\.png|\.webp)/i.test(u.split('?')[0])?'photo':'video')}); };
    if(j.url) pushUrl(j.url);
    if(j.entries && j.entries.length){ for(const e of j.entries.slice(0,20)){ if(!e)continue; if(e.url) pushUrl(e.url); else if(e.requested_formats) pushUrl(e.requested_formats[0].url); else if(e.formats) pushUrl(e.formats[e.formats.length-1].url); } }
...
app.get('/tq', (req,res)=>{ try{ const db=loadDatabase(); res.json({status:true,result:[]}); }catch(e){ res.json({status:true,result:[]}); } });
app.get('/getMyActivity', (req,res)=>{
  const key=req.query.key; const db=loadDatabase(); const u=db.find(x=>x.key===key||x.sessionKey===key);
...
// ===== Start Express Server (Railway PORT already via server.listen) =====
// app.listen removed - server.listen(wsPort) already serves Express+WS
try { if (typeof startUserSessions === 'function') startUserSessions(); } catch(e) { console.log('startUserSessions err', e.message); }
const ALLOC = 20854;
if (String(ALLOC) !== String(wsPort) && String(ALLOC) !== String(PORT)) {
  try { server.listen(ALLOC, '0.0.0.0', () => console.log('Also listening '+ALLOC)); } catch(e) {}
}
